...
const defaultConfig = {
  ...
  type: [],
  multiple: true,
  apiUrl: (): Promise<UploadResponseType> => {
  ...
  async newValue => {
    try {
      ...
      const list = await Promise.all(
        mergedConfigRef.value.multiple ? data : data[0] ? [data[0]] : []
      );
...
const beforeUpload = (file: File) => {
  ...
  if (
    file.type &&
    mergedConfigRef.value.type.length > 0 &&
    !mergedConfigRef.value.type.includes(file.type)
  ) {
  ...
  if (
    !file.type &&
    mergedConfigRef.value.type.length > 0 &&
    !mergedConfigRef.value.type.includes(suffix)
  ) {
    message.error(`文件不符合:${mergedConfigRef.value?.type.join()}`);
    return false;
  }
